 ...
 * than the host's. The timezone is stated rather than assumed — a mis-set
 * clock is the classic way to book a call nobody shows up to.
 */
...
export default function TimePicker({
  ...
  if (!slots.length) {
    return (
      <div className="rounded-[12px] border border-paper/15 bg-paper/[0.04] px-4 py-6 text-center">
        <p className="m-0 text-sm text-paper/70">
          No times are open right now. Try again shortly, or email us directly.
        </p>
      </div>
    );
  }
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-3 flex items-center justify-between">
            <button
              ...
              className="cursor-pointer rounded-full px-2 py-1 text-paper/50 transition-colors duration-150 hover:text-paper disabled:cursor-default disabled:text-paper/15"
            >
              ‹
            </button>
            <span className="font-sans text-[15px] [font-weight:580] text-paper">
          ...
          <div className="grid grid-cols-7 gap-1">
            {DOW.map((d, i) => (
              <div
                key={i}
                className="pb-1 text-center font-mono text-[10px] tracking-[0.08em] text-paper/40"
              >
                {d}
              </div>
            ))}
                ...
                <button
                  ...
                  className={`aspect-square rounded-full text-[13px] transition-colors duration-150 ${
                    on
                      ...
                      : open
                        ? "cursor-pointer bg-accent-soft/[.10] font-semibold text-accent-soft hover:bg-accent-soft/20"
                        : "cursor-default text-paper/20"
                  }`}
                >
                  {d.getDate()}
                </button>
          ...
          {/* Duration lives in the panel header now — this line only states
              whose clock the times are on. */}
          <p className="m-0 mt-4 font-mono text-[10px] tracking-[0.06em] text-paper/40">
            Times shown in {BROWSER_TZ.replace(/_/g, " ")}
          </p>
        </div>

        {/* ---- times for the chosen day ----
            Flex sizes a row to its TALLEST item, and a long list of times
            beats the calendar every time — left in flow it sets the row
            height, the calendar stretches to match, and you get a dead gap
            under the grid. flex-1 can't rein it in either: a 0% basis against
            an indefinite height collapses back to content size.
            ...
            row, this column stretches to that, and the list scrolls inside
            it. Below sm the column stacks with no calendar beside it to
            measure against, so it stays in flow and falls back to max-h. */}
        <div className="sm:relative sm:w-[136px] sm:flex-none">
          <div className="flex flex-col sm:absolute sm:inset-0">
            <p className="m-0 mb-2 flex-none font-mono text-[10px] uppercase tracking-[0.08em] text-paper/40">
              {day ? dayFmt.format(new Date(`${day}T12:00:00`)) : "Pick a day"}
            </p>
            {/* data-lenis-prevent: Lenis runs in root mode and eats the wheel
                for the window, so a nested scroller never sees it — without
                this the page scrolls instead of the list. Only bites with a
                discrete mouse wheel; Lenis stands down for trackpads. */}
            <div
              data-lenis-prevent
                ...
                return (
                  <button
                    ...
                    className={`flex-none cursor-pointer rounded-[9px] border py-2 text-center font-mono text-[12px] tracking-[0.06em] transition-colors duration-150 ${
                      on
                        ? "border-accent-soft bg-accent-soft/[.14] text-accent-soft"
                        : "border-paper/15 bg-paper/[0.04] text-paper/70 hover:border-accent-soft/50 hover:text-paper"
                    }`}
                  >
