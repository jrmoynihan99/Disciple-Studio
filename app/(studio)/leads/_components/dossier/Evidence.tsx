...
import type { QuestionView, SubSignal, VerdictState } from "@/lib/leads/engine/types";
...
export function EvidenceBody({ q }: { q: QuestionView & Record<string, unknown> }) {
  ...
  return (
    <div className="text-[12.5px] leading-relaxed text-lead-ink2">
      {/* `q.label` is deliberately NOT rendered here. Every call site already
          ...
          sentence twice. */}
      {typeof q.evidence === "string" && q.evidence && <p className="my-1">{q.evidence}</p>}

      {/* The q1 `unverified` panel: what the model CLAIMED, the closest text
          actually on the page, and the similarity — with the ANSWER WITHHELD.
          No real church has ever produced this state; it is built against the
          synthetic record, and it is unreachable in the reference build. */}
      {q.answer === "unverified" && (
      ...
      {parentUrl && kind !== "url" && !q.quote && (
        <SourceLine url={parentUrl} unread={unread} />
      )}
    </div>
  );
}
