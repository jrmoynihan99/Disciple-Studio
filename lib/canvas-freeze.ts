 ...
 * drops from the `::view-transition-old/new(root)` image. So any canvas
 * effect blanks out the instant a page transition starts, even though the
 ...
 * shows the drawing instead of nothing. The leaving page unmounts moments
 * later so its baked canvases are thrown away; unfreeze() is only a safety
 * net for any canvas that happens to outlive the transition.
 *
 * (No-op on pages without <canvas> — kept so canvas effects added later
 * survive transitions without anyone having to rediscover this.)
 */

// Cap the baked image so toDataURL stays cheap at click time — the frozen
// frame only shows for the length of the transition, so downscaling is
// invisible.
const MAX_DIM = 900;
...
export function freezeCanvasLayers(): void {
  ...
  document.querySelectorAll<HTMLCanvasElement>("canvas").forEach((c) => {
    ...
    // off-screen won't show in the transition — skip the toDataURL cost.
